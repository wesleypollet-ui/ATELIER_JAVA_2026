...
<head>
    ...
    <style>
        ...
        .calendar .day{padding:6px;border-radius:6px;background:var(--card);border:1px solid rgba(15,23,36,0.04);min-height:60px}
        .calendar .muted{color:var(--muted);opacity:0.6}
...
<div class="container">
    ...
    <%
    // Server-side task model + persistence (same as before)
    %>
...
(function(){
    ...
    function render(){
        ...
        for(var d=1; d<=daysInMonth; d++){
            ...
            // tasks count
            var dateStr = e.dataset.date;
            var count = tasksData.filter(function(t){ return t.dueDate===dateStr; }).length;
            if(count>0){ var dot=document.createElement('span'); dot.className='date-dot'; e.appendChild(dot); var cspan = document.createElement('span'); cspan.style.marginLeft='6px'; cspan.style.fontSize='0.85rem'; cspan.textContent = (count>1?count+' tâches':count+' tâche'); e.appendChild(cspan); }
            e.addEventListener('click', function(){ showTasksFor(this.dataset.date); });
            calendarEl.appendChild(e);
        }
    }

    function showTasksFor(date){
        var filtered = tasksData.filter(function(t){ return t.dueDate===date; });
        if(filtered.length===0){ tasksForDay.innerHTML = '<p>Aucune tâche pour '+date+'</p>'; return; }
        var html = '<h3>Tâches pour ' + date + '</h3><ul>';
        filtered.forEach(function(t){ html += '<li><strong>'+escapeHtml(t.title)+'</strong> - '+escapeHtml(t.description)+' '+(t.completed?'<em>(Terminé)</em>':'')+'</li>'; });
        html += '</ul>';
